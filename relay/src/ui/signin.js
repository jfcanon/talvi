// The custom Clerk sign-in page — "/sign-in". Ported from the blue release
// (s7/talvi-blue-auth-handover.md §4) — the settled flow, moved not rewritten:
// custom UI, strict CSP with a nonce, no unsafe-inline, no unsafe-eval in
// prod. Also the OAuth return page — "/sso-callback": Clerk redirects the
...
//
// Paths are PREFIX-aware: the relay mounts at app.ygdcbtmc4u.uk/relay, so the
// sign-in page lives at /relay/sign-in, redirects home to /relay/, and points
// clerk-js's OAuth return at /relay/sso-callback. The /relay prefix is the
// ONLY difference from the blue copy; the auth flow and CSP are byte-identical.
//
...
export function signInPage({ publishableKey, nonce }) {
  ...
  return (
    ...
    '<link rel="stylesheet" href="' + PREFIX + '/s.css?v=' + v + '">' +
